...
import {LayoutDashboard, ListChecks, Save, User, Users} from "lucide-react";
...
const SandboxTestResultPage = () => {

    const BREADCRUMBS: Crumb[] = [
        {label: "Dashboard", icon: LayoutDashboard, href: AdminDashboardController.url()},
        {label: "Sandbox Results", icon: Users, href: ViewSandbox.url()},
        {label: `Test result`, icon: ListChecks}
    ];

    const {sandboxTest} = usePage().props
    ...
    return (
        <PractitionerPortalLayout>
            <Head title="Sandbox test result"/>
            ...
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <p className="mt-1.5 flex items-center gap-1.5 text-sm text-emerald-700/80">
                        View sandbox test results trialed on a client's nutrition rules.
                    </p>
                </div>
            ...
            <div className="grid xl:grid-cols-2 gap-5">
                <div className="space-y-4">
                    <div className="bg-white rounded-2xl border border-sage-100 space-y-2 p-5">
                        <div>
                            <h2 className="text-sm font-semibold text-sage-700 mb-1">Client</h2>
                            <span className="font-medium text-green-800 text-sm">{sandboxTest?.client.name}</span>
                        </div>
                        <h2 className="text-sm font-semibold text-sage-700 mb-1">Conditions</h2>
                        <span className="font-medium text-green-800 text-sm">{sandboxTest?.test_profile?.conditions?.join(' · ')}</span>
                    </div>
                    <div className="bg-white rounded-2xl border border-sage-100 overflow-hidden">
                        <div className="px-5 py-3 border-b border-sage-100 text-sm font-medium text-sage-700">Rules
                        </div>
                        <div className="p-3 space-y-2">
                            {
                                sandboxTest?.rule_set_snapshot.map((rule: any) => (
...
export default SandboxTestResultPage;
